...
# Event types whose attached filing PDF is worth an inline the LLM read — the
...
def _enrich_event_docs(results: dict[str, list[alerts.Alert]], cap: int = 25) -> None:
    """Download + the LLM-analyse the attached filing for EVERY notable doc-bearing
...
def run_watchlist_scan(con: duckdb.DuckDBPyConnection | None = None) -> ScanResult:
    ...
    try:
        ...
        _enrich_event_docs(results)                         # inline the LLM read of filings
