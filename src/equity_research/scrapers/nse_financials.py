...
class ParsedXBRL:
    ...
    context_dates: dict[str, tuple[date | None, date | None]]  # id -> (start, end)
...
def parse_result_xbrl(raw: bytes) -> ParsedXBRL:
    ...
    Only dimensionless duration facts are kept (the headline P&L, not the
    by-axis breakdowns). Facts are keyed by context ID — see the convention note
    above on why dates can't be trusted.
    """
    root = etree.fromstring(raw)

    # Index contexts: id -> (start, end, has_dimensions).
    meta: dict[str, tuple[date | None, date | None, bool]] = {}
    for ctx in root.findall(f"{{{_XBRLI}}}context"):
        ...
        meta[ctx.get("id")] = (start, end, has_dims)
    ...
    for el in root.iter():
        ...
        if ctx_id not in meta or el.text is None:
            continue
        start, end, has_dims = meta[ctx_id]
        if has_dims or end is None:          # skip dimensional breakdowns + instants
            continue
        try:
            value = float(el.text)
        except ValueError:
    ...
    dates = {cid: (s, e) for cid, (s, e, _) in meta.items()}
    return ParsedXBRL(nature=nature, facts_by_context=facts, context_dates=dates)
