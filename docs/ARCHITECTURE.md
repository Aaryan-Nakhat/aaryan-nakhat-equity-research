...
technical / valuation analysis → the LLM writes the thesis → delivered to Telegram
...
self-healing gate: first heartbeat >=18:00 IST, once per trading day (already_scanned_today)
        ...
        ▼  scan.run_watchlist_scan() → ScanResult(results, movers, upcoming):
              ...
              5. _enrich_event_docs(): download + the LLM-read notable filings (inline, capped 5)
...
| **LLM** | synthesis + name resolution | the configured LLM via **the provider** (service account) |
