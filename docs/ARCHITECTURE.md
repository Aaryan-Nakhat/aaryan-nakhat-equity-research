...
technical / (sector-appropriate) valuation analysis + signals → the LLM writes the thesis →
...
```
   ...
   └───────────────────────────┬──────────────────────────────────────────┬──────────────┘
                   ...
                   │  resolve.py  "name" → NSE symbol (the LLM + Google Search)   │
...
You ▶ Telegram: "example power"  (or "Reliance consolidated")
        │
        ▼  resolve.py  → the LLM+Search → NSE symbol(s)
   one match? ──run──┐        several? ──▶ buttons ──▶ you tap one ──┐
        ...
        pipeline.generate_report():  ensure financials ingested (on-demand)
              ...
              → synthesize.py  → the LLM forensic write-up
        │
...
self-healing gate: first heartbeat >=18:00 IST, once per trading day (already_scanned_today)
        ...
        ▼  scan.run_watchlist_scan() → ScanResult(results, movers, upcoming, market, insider):
              ...
              5. _enrich_event_docs(): download + the LLM-read notable filings (inline)
...
| **LLM** | synthesis + filing/guidance extraction + name resolution | the configured LLM via **the provider** (service account) |
