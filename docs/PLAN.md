...
- **Events** — bulk/block **institutional deals**, a defined **corporate-event taxonomy**
  ...
  first-sight seeding, and **inline the LLM analysis** of notable filing PDFs (capped 5).
...
- **Auto multi-filing read** (`pipeline._filings_for_analysis`): every report feeds the LLM
...
- **More market-header context** (extends the sectoral-indices/VIX/FII-DII header): **USD/INR**,
  the **10-yr G-Sec yield**, and **gold / silver / crude**. Sources are bot-walled (MCX/Akamai)
  or JS-app (RBI/FBIL) and need dedicated scraping work — deferred. Global cues (US/Asia/Brent/
  DXY) would need a third-party vendor, which the primary-sources-only rule excludes.
...
(the LLM) adds value on the **unstructured** side:
...
the provider AI / the LLM Developer API) · email delivery.
