...
- `/watch <name>` — resolve (the LLM+search) → add → ingest financials → seed state
...
- **📅 Upcoming:** the watchlist's events in the next ~35 days — board-meeting /
  ...
  LLM-labelled** in one batched the LLM call (`synthesize.label_events`) into clean
...
- **Events (filed today):** the watchlist's non-routine corporate filings dated *today*
  (`corporate_announcements_batch` → `alerts._categorise`); **no** heavy per-PDF the LLM read
