...
it to NSE symbol(s) [local map, else the LLM+Google-Search]; if several match it
shows buttons to pick one; then it ingests-on-demand, builds the deep brief,
runs the the LLM forensic analysis, and replies with the report (inline + a .md
file).
...
plus the the LLM/the provider vars. See .env.example.
"""
