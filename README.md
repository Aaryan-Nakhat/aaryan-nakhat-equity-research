...
- **Mutual funds** *(new track)* — AMFI NAV backbone (`mf_scheme`/`mf_nav`, accumulated daily).
  ...
  **benchmark-relative alpha/beta/up-down-capture/tracking-error** vs the category's index
  (~5y history backfilled from NSE archives) · category percentile · and a **the LLM verdict**
...
- **Report** — the LLM reads the quant brief (+ filing PDFs) and writes a forensic thesis,
...
valuation + signals → the LLM report → email **or** Telegram bot, always-on). On-demand
...
- [`docs/TECHNICAL.md`](docs/TECHNICAL.md) — indicators. [`docs/REPORTS.md`](docs/REPORTS.md) — the LLM synthesis, Telegram bot, PDF, email.
...
- **the LLM** — either the provider AI (a GCP service-account JSON, also gitignored) or a
  Developer API key.
